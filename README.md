...
2. Entre na pasta principal do projeto:
   ...
   cd raizes
   ```
3. Instale as dependências e execute o projeto utilizando o Maven Wrapper que já vem incluso:
   ```bash
...
A documentação visual e interativa de todos os endpoints e contratos da API foi gerada automaticamente [cite: 18]. Com a aplicação rodando, acesse o link abaixo no seu navegador para visualizar e testar:

👉 [Acessar Swagger UI no Navegador](http://localhost:8080/swagger-ui/index.html)
...
2. **Ordem de Execução Sugerida:**
   * **1º Passo:** Execute a requisição `T03 - Login com Sucesso` utilizando as credenciais cadastradas pelo *seed* (email: `cliente@exemplo.com`, senha: `Senha@123`).
