...
Instale as dependências e execute o projeto utilizando o Maven Wrapper que já vem incluso:
A API estará rodando localmente na porta padrão: http://localhost:8080
📄 Documentação da API (Swagger/OpenAPI)
A documentação visual e interativa de todos os endpoints e contratos da API foi gerada automaticamente. Com a aplicação rodando, acesse a URL abaixo no seu navegador:
...
Abra o Postman e vá em Import para importar o arquivo .json da coleção.
Ordem de Execução Sugerida:
1º Passo: Execute a requisição T03 - Login com Sucesso utilizando as credenciais cadastradas pelo seed.
2º Passo: Copie o accessToken gerado na resposta.
...
Validação do Fluxo Crítico: O cenário T01 realiza a criação do pedido garantindo a exigência de Multicanalidade (com o envio de "canalPedido": "APP"), faz a validação de estoque, aciona o Mock de Pagamento aprovado (T04) e retorna o pedido criado.
Tratamento de Erros: Estão inclusos cenários negativos tratando indisponibilidade de estoque (T09 - Status 409) e falha no mock de pagamento (T08 - Status 400), todos retornando um JSON padronizado de erro.
